...
Our project with Neuraville is a circuit capable of counting shapes from static input. The following is a description of the components of the circuit we used to accomplish this task.
...
Next, we think about how vision sampler should react to this new connection. Assuming you have firing neurons in the vision\_c our new cortical area should be an exact match if all is working properly. Instead, we want it to display only the neurons we are intending to sample. To make this happen we take advantage of the Neuron Firing Parameters, in particular the Neuron Firing Threshold. By default, vision\_c’s PSP should be set to 1.0; change vision sampler’s firing threshold to 36\. Now, there are no neurons firing in the vision sampler, this is because vision\_c’s signal simply isn’t strong enough to cross this threshold. The reason for this will come to light with the next cortical area.

The second cortical area handles the logic of the parsing motion we are looking for. I refer to it as the scanner cortical area, which is connected from its output to the vision sampler via a projector connection. Scanner’s PSP is set to 35 and PSP Uniformity is turned on. When the area’s 35 PSP is projected onto the vision sampler, it adds onto the neurons vision\_c is exciting with a PSP of 1, resulting in a total PSP of 36, crossing the vision sampler’s firing threshold wherever the scanner is actively parsing.

Continuing with the scanner, the size of this area directly impacts how many areas the vision\_c is being segmented into and the size of shapes that can be fully seen. Our 4x4x1 implementation segments vision\_c into 16 areas, one for each neuron in the scanner. This then translates into a 32x32 box scanning over the input. Any shape that does not fully fit into this area will not be recognizable later down the pipeline. The scanner’s cortical area should have the following settings: MP Accumulation turned on, a firing threshold to 100, and a Consecutive Firing Count of 40\. The following connections implement the core operating logic of the scanner.

We will implement four recursive connections from the scanner to itself. 

1. Sweeper : This is a built-in connectivity rule in FEAGI that handles the core logic of scanning. Default settings are used.  
2. Last\_to\_first : Another built-in connectivity rule in FEAGI which ensures the sweeper movement repeats the full sequence when reaching the last neuron. We used this connection for testing and developing the scanner; when actually counting it is removed so that the shapes aren’t counted multiple times.  
3. Block\_to\_block: Another built-in connectivity rule in FEAGI that we will not leave default. PSP Multiplier is set to 10.0000.  
4. Inhibit\_prior: The final connection which will ensure only one neuron fires at a time, creating a clean scan of the entire area. This is a custom vector connectivity rule created with the following three vectors. PSP Multiplier is set to 10 and Inhibitary is toggled on.  
   1. X=-1 Y=0 Z=0  
   2. X=3 Y=-1 Z=0  
   3. X=3 Y=3 Z=0

In order to actually start scanning we apply an initial bit of PSP to start the continuous process. Create a third cortical area which will be referred to as start, it is a simple 1x1x1 area with PSP Uniformity turned on, a PSP of 255.0, a PSP Max of 35.00, and a Firing Threshold of 0.10. It only has a single connection leading from it to scanner. It is a custom connection which we call scan\_start. It is a Patterns-type connectivity rule with a source of \[\*, \*, \*\] and a destination of \[1, 1, 0\].

To start the scanner all the user has to do is manually select the one neuron in start with a shift+click and press the space bar to manually fire it.

Next we implement an area that can extract the data we are scanning so other circuits can use it. Let’s make another cortical area called feature extraction. We set is as a 32x32x1 sicne that is the size of the sampled area created by scaner. We set the connection from vision sampler to feature extraction using the tile connectivity rule so that whatever is firing in vision sampler will fill the entirety of feature extraction. Now, whatever potential is sent out from the scanning circuit will be easily handled by later circuits.
...
Video Link: https://drive.google.com/file/d/1ZfhQnJvL44RFA1QvY6m_lISMfaLMLSLq/view?usp=drive_link
